import React from "react";
import Tree from '@naisutech/react-tree'
import axios from "axios";
import config from 'react-global-configuration';

class DirectoryPickerDialog extends React.Component {
    constructor(props) {
        super(props);
        this.props = props;
        this.data = [];
        this.dir_img = <img src="data:image/png;base64," />;
        this.file_img = <img src="data:image/png;base64," />;
        this.selectedNodePath = "";
        this.selectedNodeNo = -1;
    }

    state = {
        data: []
    };

    // Directory IDs: 0, 1, 2,...
    // File IDs: string (full path)
    onSelect = selectedNode => {
        var url = config.get("django_url" + "/common/rest");

        if (Number.isInteger(selectedNode[0])) {
            // Check if it's directory or file
            if (selectedNode[0] > 0) {
                if (this.state.data[selectedNode[0]]["loaded"] == 0) {
                    // Process directory update
                    var full_path = this.state.data[selectedNode[0]]["path"];
                    var next_id = this.state.data.length;

                    this.selectedNodePath = full_path;
                    this.props.setCreateBasePath(this.selectedNodePath)

                    this.selectedNodeNo = selectedNode[0];

                    var url = config.get("django_url") + "/common/rest";

                    axios
                        .get(url, { params: { req: "GET_DIRECTORY_CONTENT", path: full_path, parent: selectedNode[0], next_id: next_id } })
                        .then((res) => {
                            var files = res.data.files;
                            var dirs = res.data.dirs;

                            // Copy current data to new object
                            let curr_data = JSON.parse(JSON.stringify(this.state.data));

                            for (var i = 0; i < files.length; i++) {
                                curr_data[selectedNode[0]].items.push(files[i]);
                            }

                            for (var i = 0; i < dirs.length; i++) {
                                curr_data.push(dirs[i]);
                            }

                            curr_data[selectedNode[0]]["loaded"] = 1;

                            this.setState({ data: curr_data });
                        })
                        .catch((err) => alert(err));
                } else {
                    this.selectedNodeNo = selectedNode[0];
                    if (selectedNode[0] > 0) {
                        this.selectedNodePath = this.state.data[selectedNode[0]].path;
                        this.props.setPath(this.selectedNodePath);
                        this.props.setCreateBasePath(this.selectedNodePath)
                    } else {
                        this.selectedNodePath = ""
                        this.props.setPath("");
                        this.props.setCreateBasePath("")
                    }
                }
            }
        } else {
            // Selected node is file
            // No extra actions required
            if (selectedNode.length > 0) {
                this.selectedNodePath = selectedNode[0];
            }

            if (selectedNode[0] != undefined) {
                if (selectedNode[0].slice(-4).toString().toLowerCase() == ".png") {
                    var url = config.get("django_url") + "/common/rest";
                    axios
                        .get(url, { params: { req: "GET_IMAGE_URL", path: selectedNode[0] } })
                        .then((res) => {
                            var redirectURL = config.get('django_url') + res.data.url;
                            window.open(redirectURL, '_blank').focus();
                        })
                }
            }
        }
    }

    componentDidMount() {
        // Get directory structure from Django
        //var url = config.get("django_url") + "/common/rest";
        var url = config.get("django_url") + "/common/rest";
        axios
            .get(url, { params: { req: "GET_ROOT_CONTENT" } })
            .then((res) => {
                this.setState({ data: res.data });
            })
            .catch((err) => alert(err));
    }

    componentWillUnmount() {
        this.props.setPath(this.selectedNodePath);
    }

    componentDidUpdate(prevProps) {
        if (this.props.createFolder != prevProps.createFolder) {
            // Copy current data to new object
            let currData = JSON.parse(JSON.stringify(this.state.data));
            var newDir = {
                id: this.state.data.length,
                items: [],
                label: this.props.createFolder,
                loaded: 0,
                parentId: this.selectedNodeNo,
                path: this.selectedNodePath + "/" + this.props.createFolder
            }

            currData.push(newDir)
            this.setState({ data: currData });
        }
    }

    render() {
        return (
            <>
                <div className='directory-picker'>
                    <Tree
                        nodes={this.state.data}
                        onSelect={this.onSelect}
                        theme="light"
                        IconRenderer={({ label }) => {
                            return label === 'leaf' | 'node' ? this.file_img : this.dir_img
                        }} />
                </div>
            </>
        );
    }
}

export {
    DirectoryPickerDialog
};